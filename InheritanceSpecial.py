...
#Special discount for any particular class
    def apply_special_discount(self, special_discount_rate: float):
        """Applies a special discount using a passed discount rate for this item."""
        assert 0 <= special_discount_rate <= 1, "Discount rate must be between 0 and 1"
        self.price = self.price * (1 - special_discount_rate)
        print(f"Special discount applied for {self.name}. New price: {self.price}")
...
item2.apply_special_discount(0.4)
item3 = ElectronicItem("Laptop", 1500, 2)
print(f"{item3.name} Total Price: ${item3.calculate_total_price()}")
item3.apply_discount()
print(f"Price after discount for per {item3.name}: ${item3.price}")
